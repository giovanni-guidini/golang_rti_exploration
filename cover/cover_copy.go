...
func main() {
	...
	if *mode != "" {
		fmt.Println("Enter name of file to annotate")
		var fileName string
		fmt.Scanln(&fileName)
		annotate(fileName)
		return
...
func annotate(name string) {
	...
	// file.prettyPrint()
	// printer.Fprint(os.Stdout, file.fset, file.astFile)
...
func (f *File) addVariables(w io.Writer) {
	...
	fmt.Fprintf(w, "\nvar %s = struct {\n", *varVar)
	fmt.Fprintf(w, "\tCount     []uint32\n")
	fmt.Fprintf(w, "\tPos       []uint32\n")
	fmt.Fprintf(w, "\tNumStmt   []uint16\n")
	fmt.Fprintf(w, "} {\n")

	// Initialize the Count slice
	fmt.Fprintf(w, "\tCount: make([]uint32, %d),\n", len(f.blocks))
